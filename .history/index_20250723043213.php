...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['add_app'])) {
        $newApp = [
            ...
            'api_token' => generateApiToken(16) // 32 hex chars
        ];
        $apps[] = $newApp;
        saveJson($appsFile, $apps);
        logAction("App Added: " . $newApp['name']);
    }
    ...
    if (isset($_POST['regen_token'])) {
        foreach ($apps as &$app) {
            if ($app['id'] === $_POST['app_id']) {
                $app['api_token'] = generateApiToken(16);
                logAction("API Token regenerated for app: " . $app['name']);
                break;
            }
        }
        saveJson($appsFile, $apps);
...
// Load logs
$logs = loadJson($dataDir . '/logs.json');
?>

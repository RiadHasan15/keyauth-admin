...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['add_key'])) {
        ...
        $stmt = $mysqli->prepare("INSERT INTO license_keys (license_key, app_id, banned, hwid_banned) VALUES (?, ?, 0, 0)");
        ...
        <table class="table table-bordered table-hover" id="keysTable">
            ...
            <?php foreach ($paginatedKeys['items'] as $key): ?>
                <tr>
                    ...
                    <td><?= isset($key['hwid_banned']) && $key['hwid_banned'] ? '<span class="text-danger">Banned</span>' : '<span class="text-success">Active</span>' ?></td>
                    <td class="table-actions">
                        ...
                        <form method="POST" style="display:inline;">
                            <input type="hidden" name="key_id" value="<?= $key['id'] ?>" />
                            <button class="btn btn-sm btn-secondary" name="toggle_hwid_ban">
                                <?= isset($key['hwid_banned']) && $key['hwid_banned'] ? 'Unban HWID' : 'Ban HWID' ?>
                            </button>
                        </form>
                    </td>
                </tr>

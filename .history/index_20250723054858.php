...
// Load data
$apps = $mysqli->query("SELECT * FROM apps")->fetch_all(MYSQLI_ASSOC);
$keys = $mysqli->query("SELECT license_keys.*, apps.name AS app_name FROM license_keys LEFT JOIN apps ON license_keys.app_id = apps.id ORDER BY license_keys.id DESC")->fetch_all(MYSQLI_ASSOC);

...
// Debug: Log the key data to see what we got
...
// Log HWID usage - with additional validation
$hwidForLog = $hwid ?? '';
$licenseKeyId = (int)$keyData['id'];

log_msg("About to insert HWID log - License Key ID: $licenseKeyId, HWID: $hwidForLog, IP: $ip");

// Verify the license key ID exists before inserting
$stmt = $conn->prepare("SELECT id FROM license_keys WHERE id = ?");
if (!$stmt) {
    log_msg("Prepare failed (verify key): " . $conn->error);
    send_response(['success' => false, 'message' => 'Internal error']);
}
...
if ($verifyResult->num_rows === 0) {
    log_msg("License key ID does not exist in database: $licenseKeyId");
    send_response(['success' => false, 'message' => 'License key reference error']);
}

$stmt = $conn->prepare("INSERT INTO hwid_logs (license_key_id, key_id, hwid, ip_address) VALUES (?, ?, ?, ?)");
if (!$stmt) {
    log_msg("Prepare failed (hwid_logs): " . $conn->error);
    send_response(['success' => false, 'message' => 'Internal error']);
}
$stmt->bind_param("iiss", $licenseKeyId, $licenseKeyId, $hwidForLog, $ip);

if (!$stmt->execute()) {
    log_msg("Failed to insert HWID log: " . $stmt->error);
    send_response(['success' => false, 'message' => 'Failed to log HWID usage']);
}

...
ini_set('error_log', __DIR__ . '/../logs/php_error.log'); // error log path
...
function send_response($data) {
    if (ob_get_length()) {
        ob_end_clean(); // clear and turn off output buffering
    }
...
if ($hwid !== null) {
    if (empty($keyData['hwid'])) {
        ...
        if (!$stmt) {
            log_msg("Prepare failed (hwid update): " . $conn->error);
            send_response(['success' => false, 'message' => 'Internal error']);
        }
...
// Log HWID usage - check for null HWID, use empty string if null
$hwidForLog = $hwid ?? '';

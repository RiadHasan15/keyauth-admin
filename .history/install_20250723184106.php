...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($db_host && $db_user && $db_name && $site_url && $admin_email && $admin_username && $admin_password) {
        ...
        if ($mysqli->connect_error) {
            ...
            // Create admin_users table with email column
            $mysqli->query("
            ...
            $checkColumn = $mysqli->query("SHOW COLUMNS FROM admin_users LIKE 'email'");
            if ($checkColumn->num_rows === 0) {
                $mysqli->query("ALTER TABLE admin_users ADD COLUMN email VARCHAR(255) NOT NULL AFTER password");
            }

            // Drop settings table if it exists to ensure correct columns
